 ...
 */
 ...
 var PopupThree = function(options) {
  ...
  $(document).on('click', this._trigger, this._onClickTrigger.bind(this));
  if (this._closeonbackgroundclick) {
      $(document).on('click', '.c-popup_three__layout', this._onClickClose.bind(this));
  }
}
 
PopupThree.prototype._onClickTrigger = function(e) {
    e.preventDefault();
    if (!this._template) return false;
    var idPopup = $(e.target).attr('href');
    if ($(idPopup).size() == 0) { 
        this._creatPopup(e.target, idPopup);
    } else {
        this._show(idPopup);
    }    
}

PopupThree.prototype._onClickClose = function(e) {
  e.preventDefault();
  this._close();
} 

PopupThree.prototype._creatPopup = function(elem, id) {
  var self = this; 
  var idTmp = $(elem).data('tmp');
  var idTmpPopup = $(elem).data('tmp-content');
  var tmpPopup = $(idTmp).html(); 
  var tmpContentPopup = $(idTmpPopup).html(); 
  var compiled = _.template(tmpPopup, { content: tmpContentPopup, id: id.slice(1)});
  $('body').append(compiled);
  this._show(id);
}

PopupThree.prototype._show = function(id) {
  var popup = $(id);
  popup.css({ top: '-' + popup.outerHeight() + 'px' }).show();
  this._setCoords(popup);
  $('.c-popup_three__layout').show();
  
  popup.on('click', this._closer, this._onClickClose.bind(this));
}

PopupThree.prototype._close = function() {
    switch (this._animation) {
    case 'fade':
      $('.c-popup_three').animate({ opacity: 0 }, this.__animationSpeed);
      $('.c-popup_three__layout').animate({ opacity: 0 }, this.__animationSpeed, this._hide);
      break;
    case 'slideup':
      $('.c-popup_three').animate({
        top: '-' + $('.c-popup_three').outerHeight() + 'px',
        opacity: 1 
      }, this.__animationSpeed, this._hide);
      $('.c-popup_three__layout').css({ opacity: 1 });
      break;
    default:
      $('.c-popup_three').css({ opacity: 1 });
      $('.c-popup_three__layout').css({opacity: 1});
      this._hide();
  }
}

PopupThree.prototype._hide = function() {
  $('.c-popup_three').hide();
  $('.c-popup_three__layout').hide();
}

PopupThree.prototype._setCoords = function (elem) {
  var winHeight = $(window).height(),
      winWidth = $(window).width(),
      popupHeight = elem.outerHeight(),
      popupWidth = elem.outerWidth();
...
PopupThree.prototype._animate = function(elem, top, left) {
  switch (this._animation) {
    case 'fade':
      elem.css({
        top: top + 'px',
      ...
      $('.c-popup_three__layout').animate({opacity: 1}, this.__animationSpeed);
      break;
    case 'slideup':
      elem.css({
        left: left + 'px',
...
var popupThree = new PopupThree({
  trigger: '.c-popup_three__trigger',
  closer: '.c-popup__close', 
  animation: 'slideup',
  animationSpeed: 1000,
  closeonbackgroundclick: false  
